from first import plt, evaluate_spline, create_spline, PrettyTable
...
def main():
    ...
    spline_values = [evaluate_spline(
        x, nodes_x, coeff_a, coeff_b, coeff_c, coeff_d) for x in x_values]
    ...
    table = PrettyTable()
    table.field_names = ["x", "Значение сплайна",
                         "Правильное значение", "Отклонение"]

    for x, y in zip(nodes_x, nodes_y):
        spline_at_node = evaluate_spline(
            x, nodes_x, coeff_a, coeff_b, coeff_c, coeff_d)
