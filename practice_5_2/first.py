...
def create_spline(nodes_x, nodes_y):
    ...
    for i in range(1, num_intervals):
        ...
        vector_rhs[i] = 3 * ((nodes_y[i + 1] - nodes_y[i]) / intervals[i] - (nodes_y[i] - nodes_y[i - 1]) / intervals[i - 1])
    ...
    for i in range(num_intervals):
        coeff_b[i] = (nodes_y[i + 1] - nodes_y[i]) / intervals[i] - intervals[i] * (2 * coeff_c[i] + coeff_c[i + 1]) / 3
...
def main():
    ...
    for n in num_nodes:
        ...
        spline_values = [evaluate_spline(x, nodes_x, coeff_a, coeff_b, coeff_c, coeff_d) for x in x_range]

        plt.plot(x_range, function_values, label='Исходная функция f(x)', color='red')
        plt.plot(x_range, spline_values, label=f'Кубический сплайн (n={n})', linestyle='--')
        plt.scatter(nodes_x, nodes_y, color='blue')  # Узлы

        deviations = [abs(function_values[i] - spline_values[i]) for i in range(len(function_values))]
